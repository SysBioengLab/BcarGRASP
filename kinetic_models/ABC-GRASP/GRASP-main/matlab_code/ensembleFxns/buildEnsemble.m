...
%modelsBackup = cell(1,ensemble.replenishedParticles(popIdx)); %
tolScoreBackup = zeros(ensemble.replenishedParticles(popIdx), ensemble.numConditions); %
structIdxBackup = zeros(ensemble.replenishedParticles(popIdx), 1); %
xoptBackup{ensemble.replenishedParticles(popIdx),1} = []; %
simFluxesBackup{ensemble.replenishedParticles(popIdx),1} = []; %
isModelValidBackup = boolean(zeros(ensemble.replenishedParticles(popIdx), 1)); %
fminsBackup = zeros(ensemble.replenishedParticles(popIdx), ensemble.numConditions); %
timesBackup = zeros(ensemble.replenishedParticles(popIdx), ensemble.numConditions); %
...
if ensemble.parallel
    ...
    sampleCount = 0;
    nValidModels = 0;
    % Load a previously evaluated model to continue run
%     load(['backup_',ensemble.description,'.mat']) %
    ...
    while nValidModels < ensemble.numParticles && sampleCount < maxNumberOfSamples
        ...
        if contains(ensemble.description,'b_car_rejection_model_detailed')
            nSamples = ensemble.numCores*10; 
        else
            nSamples = ensemble.numCores*50; 
%         elseif ensemble.numParticles <= 100
%             nSamples = ensemble.numCores*50;        
%         elseif sampleCount == 0                                             % if no models have been sampled yet, sample ensemble.numParticles models
...
%             nSamples = round(nSamples);
        ...
        parfor ix = (sampleCount+1):(sampleCount+nSamples)
            ...
            [validModelList(ix),models(ix),strucIdx(ix),xopt{ix},tolScore(ix,:),simFluxes{ix},fmins,times] = initialSampler(ensemble, ix);
            modelsBackup(ix) = models(ix); %
            tolScoreBackup(ix,:) = tolScore(ix,:); %
            structIdxBackup(ix) = strucIdx(ix); %
            xoptBackup{ix} = xopt{ix}; %
            simFluxesBackup{ix} = simFluxes{ix}; %
            isModelValidBackup(ix) = validModelList(ix); %
            fminsBackup(ix,:) = fmins; %
            timesBackup(ix,:) = times; %
        end
        delete(gcp);
        ...
        disp(['A total of ',num2str(nValidModels),' of ',num2str(ensemble.numParticles),' particles has been achieved']) %
        disp(['A total of ',num2str(sampleCount),' of ',num2str(maxNumberOfSamples),' samples has been attempted']) %
        
        save(['backup_',ensemble.description,'.mat']) %
%         if nValidModels == 0 %% This is not necessary when using backups to cut the loop
%             break
%         end
    ...
    while ix <= ensemble.numParticles && sampleCount <= maxNumberOfSamples
        ...
        [isModelValid,model,strucIdxTemp,xoptTemp,tolScoreTemp,simFluxesTemp,fmins,times] = initialSampler(ensemble,sampleCount);

        modelsBackup(sampleCount) = model; %
        tolScoreBackup(sampleCount,:) = tolScoreTemp; %
        structIdxBackup(sampleCount) = strucIdxTemp; %
        xoptBackup{sampleCount} = xoptTemp; %
        simFluxesBackup{sampleCount} = simFluxesTemp; %
        isModelValidBackup(sampleCount) = isModelValid; %
        fminsBackup(sampleCount,:) = fmins; %
        timesBackup(sampleCount,:) = times; %
        ...
        disp(['A total of ',num2str(ix-1),' of ',num2str(ensemble.numParticles),' particles has been achieved']) %
        disp(['A total of ',num2str(sampleCount-1),' of ',num2str(maxNumberOfSamples),' samples has been attempted']) %
        save(['backup_',ensemble.description,'.mat']) %
    end
