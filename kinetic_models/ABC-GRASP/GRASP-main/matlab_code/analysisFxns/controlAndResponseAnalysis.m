...
ix_fixed     = ix_mets(end)+1:numel(ensemble.mets);
ix_enz       = ix_fixed(end)+1:freeVars;
...
for ix = startCondition:endCondition
    ...
    for jx = 1:numModels
        ...
        xfixed  = repmat(Eref,1,numel(xref));
        xenz    = repmat(Eref,1,numel(xref));
        xstep   = [xmets;xfixed;xenz];
        ...
        xfixed       = repmat(fref,1,numel(Eref));
        xenz         = repmat(Eref,1,numel(Eref));
        xenzImag     = 1i*diag(hstep_e);
        origXenzImag = xenzImag;
        
        for rxnI=1:size(ensemble.promiscuity{1},2)
            if size(ensemble.promiscuity{1}{rxnI},2) ~= 0
                sumVector = zeros(numel(Eref),1);
                for promiscuousRxnI=1:size(ensemble.promiscuity{1}{rxnI},2)
                     sumVector = sumVector + origXenzImag(:,ensemble.promiscuity{1}{rxnI}(promiscuousRxnI));
                end
        ...
        for rxnI=1:size(ensemble.promiscuity{1},2)
            if size(ensemble.promiscuity{1}{rxnI},2) ~= 0
                for promiscuousRxnI=2:size(ensemble.promiscuity{1}{rxnI},2)
                    columnsToDelete = [columnsToDelete, ensemble.promiscuity{1}{rxnI}(promiscuousRxnI)];
                end
            end
        end
        ...
        if all(abs(sum(C_v(vref~=0,:),2))-1<1e-4)
            if saveResMatrices
