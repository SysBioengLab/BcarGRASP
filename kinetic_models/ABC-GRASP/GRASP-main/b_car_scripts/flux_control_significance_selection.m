%% Test of significance for the mean flux response coefficients
% This script tests the significance of the mean flux response
...
% The output are heatmaps for the significance of the flux response
% coefficients. These are stored in
% 'GRASP-main/io/figure/flux_control_significance_selection'. Their
...
fluxSelections = {[21,22];[21,22];[21,22];[21,22];[21,22];[21,22];[21,22]};                     % fluxes to be tested
testsNs = {80;80;80;80;56;56};                                                                  % number of tests applied per ensemble

significance = 0.05;                                                                            % significance of the single test in excel
...
responseUnprocessed = controlFile.mcaResults.eResponse{1,1};
...
%% Extract reaction names and arrange flux coefficients

fluxNames = ensemble.rxns;
fluxN = length(fluxNames);
enzNames = controlFile.mcaResults.enzNames;
enzN = length(enzNames);

obsN = size(responseUnprocessed,1)/fluxN;

for i= 1:fluxN
    fluxTemp = strsplit(fluxNames{i},'r_');
    fluxNames{i} = fluxTemp{2};
end
for i= 1:enzN
...
for i = 1:obsN
    CrtBaerow = responseUnprocessed(20+(i-1)*fluxN,:);
    CrtBberow = responseUnprocessed(19+(i-1)*fluxN,:);
    CrtYaerow = responseUnprocessed(17+(i-1)*fluxN,:);
    CrtYberow = responseUnprocessed(18+(i-1)*fluxN,:);
    responseUnprocessed(17+(i-1)*fluxN,:) = CrtBaerow;
    responseUnprocessed(18+(i-1)*fluxN,:) = CrtBberow;
    responseUnprocessed(19+(i-1)*fluxN,:) = CrtYaerow;
    responseUnprocessed(20+(i-1)*fluxN,:) = CrtYberow;
end
...
response = reshape(reshape(responseUnprocessed(:,1:(end-2))',1,[]),(enzN-2)*fluxN,[])';
selectionVector = reshape(((repmat(1:1:(enzN-2),length(fluxSelection),1)+repmat((enzN-2)*(fluxSelection-1)',1,(enzN-2))))',1,[]);
response = response(:,selectionVector);
minResponse = sum(abs(min(response,[],1))>100);
maxResponse = sum(abs(max(response,[],1))>100);

enzN = enzN-2;
fluxN = length(fluxSelection);
enzNames = enzNames(1:end-2);
fluxNames = fluxNames(fluxSelection');

%% Adjust siginificance to the number of tests

%testsN = testsN+fluxN*enzN;
...
%% Plot histograms if wanted

% if ~exist(fullfile('..','io','figures','flux_control_significance_selection','histograms_values'), 'dir')
%    mkdir(fullfile('..','io','figures','flux_control_significance_selection','histograms_values'))
% end
% for ix = 1:size(response,2)
%     fluxPlot = fix((ix-1)/enzN)+1;
%     enzPlot = mod((ix-1),enzN)+1;
%     figure('visible','off')
%     histogram(response(:,ix),100,'Normalization','probability')
%     title(['Flux Response Coefficient of ',fluxNames{fluxPlot,1},' by ',enzNames{enzPlot,1}])
%     saveas(gcf,fullfile('..','io','figures','flux_control_significance_selection','histograms_values',[samplingType,'_',modelType,'_',conditionName,'_',fluxNames{fluxPlot,1},'_',enzNames{enzPlot,1},'.png']))
% end
...
% if ~exist(fullfile('..','io','figures','flux_control_significance_selection','histograms_bootstrap'), 'dir')
%    mkdir(fullfile('..','io','figures','flux_control_significance_selection','histograms_bootstrap'))
% end
% for ix = 1:size(response,2)
%     fluxPlot = fix((ix-1)/enzN)+1;
%     enzPlot = mod((ix-1),enzN)+1;
...
%     title(['Flux Response Coefficient of ',fluxNames{fluxPlot,1},' by ',enzNames{enzPlot,1}])
%     saveas(gcf,fullfile('..','io','figures','flux_control_significance_selection','histograms_bootstrap',[samplingType,'_',modelType,'_',conditionName,'_',fluxNames{fluxPlot,1},'_',enzNames{enzPlot,1},'.png']))
% end
...
pValuesSignificance = zeros(fluxN,enzN);
...
xlabel('Enzymes')
ylabel('Fluxes')
%title({'Escaled Significance of Mean Flux Control Coefficient',[modelType,' ',conditionFullName]})
...
mg.EdgeColor = 'k';
hold off
saveas(gcf,fullfile('..','io','figures','flux_control_significance_selection','png',[samplingType,'_',modelType,'_',conditionName,'.png']))
saveas(gcf,fullfile('..','io','figures','flux_control_significance_selection','jpg',[samplingType,'_',modelType,'_',conditionName,'.jpg']))
saveas(gcf,fullfile('..','io','figures','flux_control_significance_selection','svg',[samplingType,'_',modelType,'_',conditionName,'.svg']))
saveas(gcf,fullfile('..','io','figures','flux_control_significance_selection','mat',[samplingType,'_',modelType,'_',conditionName,'.fig']))

%% Create vectors for output

originalSignificances = repmat(significance,fluxN*enzN,1);
testsNVector = repmat(testsN,fluxN*enzN,1);
correctedSignificances = repmat(correctedSignificance,fluxN*enzN,1);

fluxNamesColumn = reshape(repmat(fluxNames',enzN,1),[],1);
enzNamesColumn = repmat(enzNames,fluxN,1);

%% Create Table with the results

outputTable = table(fluxNamesColumn,enzNamesColumn,originalSignificances,testsNVector,correctedSignificances,meanResponseLCI,meanResponse,meanResponseUCI,resultsSignificance,pValues);
outputTable = renamevars(outputTable,'fluxNamesColumn','Flux Control Coefficient');
...
outputTable = renamevars(outputTable,'meanResponseLCI','Mean FCC Lower CI');
outputTable = renamevars(outputTable,'meanResponse','Mean FCC Estimated');
outputTable = renamevars(outputTable,'meanResponseUCI','Mean FCC Upper CI');
...
outputFile = fullfile('..','io',"figures","flux_control_significance_selection","xls","flux_control_significance.xlsx");
