%% Plot independent fluxes of prior ensemble models
...
% 'GRASP-main/io/figures/fluxes_prior_boxplot'
...
ensembleUnfilteredDirs = {'../io/unfiltered_output/b_car_rejection_model_simple_D025.mat';
    '../io/unfiltered_output/b_car_rejection_model_regulated_D025.mat';
    '../io/unfiltered_output/b_car_rejection_model_detailed_D025.mat'};
...
% ensembleUnfilteredDirs = {'../io/unfiltered_output/b_car_rejection_model_simple_D010.mat';
%     '../io/unfiltered_output/b_car_rejection_model_regulated_D010.mat';
%     '../io/unfiltered_output/b_car_rejection_model_detailed_D010.mat'};
% 
...
ensembleUnfiltered = cell(length(ensembleUnfilteredDirs),1);

for ix = 1:length(ensembleUnfilteredDirs)
    ensembleUnfilteredFile = load(ensembleUnfilteredDirs{ix,1});
    ensembleUnfiltered{ix,1} = ensembleUnfilteredFile.ensembleUnfiltered;
end

%% Extract data

structuresN = length(ensembleUnfiltered);
conditionsN = size(ensembleUnfiltered{1,1}.populations.simFluxes{1,1},2);

metNames = ensembleUnfiltered{ix,1}.mets;
fluxNames = ensembleUnfiltered{ix,1}.rxns;
...
modelsN = zeros(length(ensembleUnfiltered),1);
for ix = 1:structuresN
    modelsN(ix,1) = length(ensembleUnfiltered{ix,1}.populations.simFluxes);
end
...
for ix = 1:structuresN
    for jx = 1:conditionsN
        for hx = 1:modelsN(ix,1)
            simulatedFluxes{jx,1}(hx,ix) = ensembleUnfiltered{ix,1}.populations.simFluxes{hx,1}(indexEX_lyc,jx);
            simulatedFluxes{jx,2}(hx,ix) = ensembleUnfiltered{ix,1}.populations.simFluxes{hx,1}(indexEX_b_car,jx);
            simulatedFluxes{jx,3}(hx,ix) = ensembleUnfiltered{ix,1}.populations.simFluxes{hx,1}(indexERG9b,jx);
        end
    end
end

 
expMeanFlux(:,1) = ensembleUnfiltered{1,1}.expFluxes(indexEX_lyc,:)';
expMeanFlux(:,2) = ensembleUnfiltered{1,1}.expFluxes(indexEX_b_car,:);
expMeanFlux(:,3) = ensembleUnfiltered{1,1}.expFluxes(indexERG9b,:);

expStdFlux(:,1) = ensembleUnfiltered{1,1}.expFluxesStd(indexEX_lyc,:)';
expStdFlux(:,2) = ensembleUnfiltered{1,1}.expFluxesStd(indexEX_b_car,:);
expStdFlux(:,3) = ensembleUnfiltered{1,1}.expFluxesStd(indexERG9b,:);
...
for ix = 1:size(simulatedFluxes,2)
    ...
    boxplot([simulatedFluxes{1,ix},simulatedFluxes{2,ix}],'Symbol','k.', 'Colors','k','PlotStyle','traditional','MedianStyle','line','Labels',boxplotNames, ...
        'OutlierSize',1,'Positions',[(1:length(modelTypes)),(1:length(modelTypes))+4],'Jitter',0.5)
    title({'Boxplot of the adjusted fluxes prior for reference ',referenceconditionFullName})
    ...
    hold off
    saveas(boxplotFigure,fullfile('..','io','figures','fluxes_prior_boxplot','png',[referenceConditionName,'_',targetFluxes{ix,1},'_boxplot.png']))
    saveas(boxplotFigure,fullfile('..','io','figures','fluxes_prior_boxplot','jpg',[referenceConditionName,'_',targetFluxes{ix,1},'_boxplot.jpg']))
    saveas(boxplotFigure,fullfile('..','io','figures','fluxes_prior_boxplot','svg',[referenceConditionName,'_',targetFluxes{ix,1},'_boxplot.svg']))
    saveas(boxplotFigure,fullfile('..','io','figures','fluxes_prior_boxplot','mat',[referenceConditionName,'_',targetFluxes{ix,1},'_boxplot.fig']))
