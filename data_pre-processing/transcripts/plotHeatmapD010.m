%% Plot heatmap of mean relative transcript concentrations at low growth rate
% This script plots the relative transcript concentrations of strains
% beta-car3 and beta-car2 at low growth rate, using the strain beta-car4 as
% reference
...
cmap_mc = [linspace(1,0.02,50)',zeros(50,1),linspace(1,0.02,50)';
    0,0,0;
    zeros(50,1),linspace(0.02,1,50)',linspace(0.02,1,50)'];

conditionFullNames = {'\beta-car2 \mu = 0.101h^{-1}';
    '\beta-car3 \mu = 0.101h^{-1}';
    '\beta-car4 \mu = 0.101h^{-1}'};
...
expressionData = readtable('output\ConditionsRelativeLimits.xlsx','Sheet','4A');

meanExpressionColumns = [2,8,14];
...
xlabel('Strains')
ylabel({'Transcripts log2-fold change';'in average relative expression'},"FontSize",8)
title({'Gene Relative Expression to ';conditionFullNames{3,1}},'FontSize',8)
set(gca,'FontSize',8,'FontName','arial')
caxis(cLimitsExpression)
colormap(expressionFigure,cmap_mc)
cb = colorbar;
set(cb,'LineWidth',1)
set(cb,'TickDir','out')
...
saveas(expressionFigure,fullfile('output\images','heatmaps','svg','heatmap_D010_4D010.svg'))
saveas(expressionFigure,fullfile('output\images','heatmaps','jpg','heatmap_D010_4D010.jpg'))
...
set(gca,'FontSize',8,'FontName','arial')
caxis(cLimitsExpression)
colormap(expressionFigure2,cmap_mc)
cb = colorbar;
set(cb,'LineWidth',1)
set(cb,'TickDir','out')
set(cb,'FontSize',8,'FontName','arial')
...
saveas(expressionFigure2,fullfile('output\images','heatmaps','svg','heatmap_D010_2D010.svg'))
saveas(expressionFigure2,fullfile('output\images','heatmaps','jpg','heatmap_D010_2D010.jpg'))

%% Plot Complete Heatmap with confidence Intervals

meanExpressionComplete = expressionData{:,[2:4,8:10,14:16]};
...
colormap(expressionFigure3,cmap_mc)
cb = colorbar;
set(cb,'LineWidth',1)
set(cb,'TickDir','out')
set(cb,'FontSize',8,'FontName','arial')
...
saveas(expressionFigure3,fullfile('output\images','heatmaps','svg','heatmap_D010_4D010_Complete.svg'))
saveas(expressionFigure3,fullfile('output\images','heatmaps','jpg','heatmap_D010_4D010_Complete.jpg'))
